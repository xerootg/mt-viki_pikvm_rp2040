...
class Plugin(BaseUserGpioDriver):  # pylint: disable=too-many-instance-attributes
    ...
    def get_pin_validator(cls) -> Callable[[Any], Any]:
        return functools.partial(valid_number, min=0, max=3, name="mtviki channel")
    ...
    def __serial_worker(self) -> None:
        logger = aioproc.settle(str(self), f"gpio-mtviki-{self._instance_name}")
    ...
    def __recv_channel(self, tty: serial.Serial, data: bytes) -> tuple[(int | None), bytes]:
        ...
        # json, the message we are looking for is {"type": "from_kvm", "status": "message_received", "port": this_is_our_int}
        found = False
        while not found:
            if tty.in_waiting:
                data = b'' # clear data
                data += tty.read_until(b"\n")
                try:
                    data = json.loads(data)
                    if data["type"] == "from_kvm" and data["status"] == "message_received":
                        channel = data["port"] - 1 # the port is 1 indexed
                        found = True
                except json.JSONDecodeError:
                    pass # ignore json decode errors, we are looking for a specific message

        return (channel, data)

    def __send_channel(self, tty: serial.Serial, channel: int) -> None:
        assert 0 <= channel <= 3
        ascii_payload = f"{channel+1}".encode("ascii")
        tty.write(ascii_payload) # just send the channel number
        tty.flush()

    def __str__(self) -> str:
        return f"Mtviki({self._instance_name})"
